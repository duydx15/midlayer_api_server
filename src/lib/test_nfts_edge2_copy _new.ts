...
export async function sendTransaction(
  ...
  const job = await queue.createJob({transactionData:signedTx})
  await job.retries(3).save()
  return new Promise((resolve, reject) => { 
    var isSuccess = false;
    var messageTxn;
    queue.process( async (job) =>{
      console.log(job.data.transactionData);
      // console.log(`Got job.data ${typeof(job.data.transactionData)}`);
      web3.eth.sendSignedTransaction(job.data.transactionData.rawTransaction)
      .then((res_:any) => {
        console.log(res_);
  ...
  //   await mutex.acquire();
  //     try {
  //       nonceGlobal.decrement(senderAddress);
  //     } finally {
  //       mutex.release();
  //     }
  // }
