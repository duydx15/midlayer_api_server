...
exports.purchasing_auto = exports.get_token_info = exports.get_token_prices = exports.get_playerID_buyer = exports.get_playerID_seller = exports.update_token_owner = exports.get_wallet_address = exports.sendETH = exports.listing_auto = exports.random_choose_token = void 0;
...
// ########  Listing sub-app
function random_choose_token(list_DB, listed, number) {
    return __awaiter(this, void 0, void 0, function* () {
        // var missing = list_DB.filter(function (item:string) { return listed.indexOf(item) < 0; });
        var missing = yield get_different_elements(list_DB, listed);
        // console.log("MISSING")
        // console.log(missing);
        // if (missing.length >0){
...
exports.random_choose_token = random_choose_token;
function get_different_elements(base_arr, check_arr) {
    return __awaiter(this, void 0, void 0, function* () {
        return base_arr.filter(function (item) { return check_arr.indexOf(item) === -1; });
    });
}
function get_stardust_tokenID_DB(con) {
    var data = new Promise((resolve, reject) => {
        // con.connect(function(err) {
        // if (err) throw err;
        con.query("SELECT stardust_tokenId FROM token_db", function (err, result, fiels) {
            if (err)
                throw err;
...
function get_stardust_tokenID_listed(con) {
    const data = new Promise((resolve, reject) => {
        // con.connect(function(err) {
        // if (err) throw err;
        con.query("SELECT stardust_tokenId FROM token_listed", function (err, result, fiels) {
            if (err)
                throw err;
            resolve(JSON.parse(JSON.stringify(result)));
            // con.end()
...
const scan_token = function () {
    return __awaiter(this, void 0, void 0, function* () {
        var tokenId_db_scan = yield get_stardust_tokenID_DB(con);
        var tokenId_listed_scan = yield get_stardust_tokenID_listed(con);
        tokenId_db_scan = tokenId_db_scan.map((singleResult) => singleResult.stardust_tokenId);
        tokenId_listed_scan = tokenId_listed_scan.map((singleResult) => singleResult.stardust_tokenId);
        ...
        if (start) {
            token_DB_arr = yield get_different_elements(tokenId_db_scan, token_listed_arr);
        ...
        if (tokenId_db_scan.length > merge_list.length) {
            const missing_token_arr = yield get_different_elements(tokenId_db_scan, merge_list);
            token_DB_arr.concat(missing_token_arr);
        }
        else if (tokenId_db_scan.length < merge_list.length) {
            const missing_token_arr = yield get_different_elements(tokenId_db_scan, merge_list);
            token_DB_arr.filter(function (item) { return missing_token_arr.indexOf(item) === -1; });
        }
...
const listing_token = function (stardust_tokenId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        new Promise((resolve, reject) => {
            ...
            con.query(`INSERT INTO token_listed (stardust_tokenId,prices,time_list) VALUES(${stardust_tokenId},${random_price},"${String(dateTime)}")`, function (err, result, fiels) {
                if (err)
                    throw err;
                resolve(result);
...
// }
var token_DB_arr = [];
var token_listed_arr = [];
var start = true;
const listing_auto = function () {
    return __awaiter(this, void 0, void 0, function* () {
        if (start) {
            yield scan_token();
            start = false;
        ...
        // console.log(token_DB_arr)
        var random = yield random_choose_token(token_DB_arr, token_listed_arr, listing_config.num_token);
        // console.log(random)
        // Check valid random
        if (random.length != 0) {
            try {
                for (var i = 0; i < random.length; i++) {
                    yield listing_token(random[i]);
                    // Update array
        ...
        setTimeout(exports.listing_auto, listing_config.time_repeat);
    });
};
exports.listing_auto = listing_auto;
...
function get_wallet_address(playID) {
    return __awaiter(this, void 0, void 0, function* () {
        const data = new Promise((resolve, reject) => {
            // con.connect(function(err) {
            // if (err) throw err;
            con.query(`SELECT address,privateKey FROM wallet WHERE playerId = '${playID}'`, function (err, result, fiels) {
                if (err)
                    throw err;
                resolve(JSON.parse(JSON.stringify(result)));
                // con.end()
...
function update_token_owner(playID_seller, playerId_buyer, stardust_tokenId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield fetch('https://polygonedge-stardust.anlab.info/api/token/transfer', options)
            .then(response => response.json())
            .then(response => console.log(response))
            .catch(err => console.error(err));
    });
}
exports.update_token_owner = update_token_owner;
function get_playerID_seller(stardust_tokenId) {
    return __awaiter(this, void 0, void 0, function* () {
        const data = new Promise((resolve, reject) => {
            // con.connect(function(err) {
            // if (err) throw err;
            con.query(`SELECT playerId FROM token_db WHERE stardust_tokenId = ${stardust_tokenId}`, function (err, result, fiels) {
                if (err)
                    throw err;
                resolve(JSON.parse(JSON.stringify(result)));
                // con.end()
...
exports.get_playerID_seller = get_playerID_seller;
function get_playerID_buyer(address) {
    return __awaiter(this, void 0, void 0, function* () {
        const data = new Promise((resolve, reject) => {
            // con.connect(function(err) {
            // if (err) throw err;
            con.query(`SELECT playerId FROM wallet WHERE address = '${address}'`, function (err, result, fiels) {
                if (err)
                    throw err;
                resolve(JSON.parse(JSON.stringify(result)));
                // con.end()
...
exports.get_playerID_buyer = get_playerID_buyer;
function get_token_prices(stardust_tokenId) {
    return __awaiter(this, void 0, void 0, function* () {
        const data = new Promise((resolve, reject) => {
            // con.connect(function(err) {
            // if (err) throw err;
            con.query(`SELECT prices FROM token_listed WHERE stardust_tokenId = ${stardust_tokenId}`, function (err, result, fiels) {
                if (err)
                    throw err;
                resolve(JSON.parse(JSON.stringify(result)));
                // con.end()
...
exports.get_token_prices = get_token_prices;
function get_token_info(stardust_tokenId) {
    return __awaiter(this, void 0, void 0, function* () {
        const data_token = yield get_playerID_seller(stardust_tokenId);
        const player_Id = data_token[0].playerId;
        // console.log("get_playerID_seller ")
        const data_wallet = yield get_wallet_address(player_Id);
        const seller_address = data_wallet[0].address;
        const seller_privateKey = data_wallet[0].privateKey;
        // console.log("get_wallet_address ")
        const listed_token_data = yield get_token_prices(stardust_tokenId);
        const prices = listed_token_data[0].prices;
        // console.log("get_token_prices ")
        return [player_Id, seller_address, seller_privateKey, prices];
    });
}
exports.get_token_info = get_token_info;
function purchasing_auto() {
    return __awaiter(this, void 0, void 0, function* () {
        if (start_purchasing) {
            yield scan_token();
        }
        ...
        const buyer_data = yield get_playerID_buyer(buyer.address);
        // console.log(buyer_data)
        const playerId_buyer = buyer_data[0].playerId;
        // const buyer_address = buyer[0].address;
        //  Choose random token
        const random_listed_token = yield random_choose_token(token_listed_arr, [], purchasing_config.num_token);
        ...
        if (random_listed_token.length != 0) {
            try {
                for (var i = 0; i < random_listed_token.length; i++) {
                    ...
                    var [player_Id_seller, seller_address, seller_privateKey, prices] = yield get_token_info(random_listed_token[i]);
                    ...
                    if (result_sendETH) {
                        ...
                        yield update_token_owner(player_Id_seller, playerId_buyer, random_listed_token[i]);
                        // Payback MS Token
...
exports.purchasing_auto = purchasing_auto;
// ########## Run app #########
// scan_token()
// listing_auto()
// listing_token(con,13)
purchasing_auto();
//# sourceMappingURL=listing_sub_app.js.map